...
public class PageUtil {
	...
	public String descriptPageLink(){
		...
		if(currentPage==1){
			...
			sb.append("<a  class='prev'  title=\"上一页\""+">&laquo; 上一页</a>");
		}else{
			sb.append("<a title=\"首页\" href=\""+descFirstUrl()+"\">&laquo; 首页</a>");
			sb.append("<a  class='prev'  title=\"上一页\" href=\""+descPreviousUrl()+"\">&laquo; 上一页</a>");
		}
		...
		if(this.currentPage== indicateTotalPages()){
			sb.append("<a  class= 'next'  title=\"下一页\""+">下一页 &raquo;</a>");
			sb.append("<a title=\"尾页\""+">尾页 &raquo;</a>");
			
		}else{
			sb.append("<a  class='next' title=\"下一页\" href=\""+descNextUrl()+"\" >下一页 &raquo;</a>");
	...
	public String descriptPageLinkMachine(){
		StringBuffer sb=new StringBuffer();
		sb.append("<div class=\""+this.style+"\">");
		sb.append("<a class='page'>" +currentPage+"/");
		sb.append(getTotalPage() +"</a>");
		if(currentPage==1){
			sb.append("<a  class='prev'  "+">&laquo; 上一页</a>");
		}else{
			sb.append("<a  class='prev'   href=\""+descPreviousUrl()+"\">&laquo; 上一页</a>");
		}
		
		if(this.currentPage== indicateTotalPages()){
			sb.append("<a  class= 'next'  "+">下一页 &raquo;</a>");
			
		}else{
			sb.append("<a class= 'next'  href=\""+descNextUrl()+"\" >下一页 &raquo;</a>");
		}
	...
	private int getTotalPage() {
	   int totalPage = 1;
	   int still= totalRecords % pageSize; 
	   if(still == 0){
